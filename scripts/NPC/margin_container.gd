...
@onready var advance_sound: AudioStreamPlayer2D = $AdvanceSound
...
func display_text(text_to_display: String, speaker_name: String = "") -> void:
	...
	_letter_index = 0
	_is_displaying = true
	if is_instance_valid(advance_sound) and advance_sound.stream != null:
		advance_sound.play()
	_display_letter()
...
func skip_to_end() -> void:
	...
	_letter_index = _full_text.length()
	_is_displaying = false
	finished_displaying.emit()
