...
@export var surge_speed   : float = 520.0
...
# ── Phase Durations ──────────────────────────────────────
@export var fly_phase_duration        : float = 5.0
@export var close_phase_duration      : float = 3.0
@export var vulnerable_duration       : float = 3.0
@export var shoot_phase_duration      : float = 6.0

# ── Action Intervals ─────────────────────────────────────
@export var fly_action_interval_min   : float = 0.5
@export var fly_action_interval_max   : float = 1.4
@export var close_action_interval_min : float = 0.3
@export var close_action_interval_max : float = 0.8

# ── Projectiles ──────────────────────────────────────────
@export var attack1_interval : float = 2.0
@export var attack2_interval : float = 3.5
@export var attack2_spread   : float = 140.0

# ── Summoning ────────────────────────────────────────────
@export var summon_interval     : float = 2.0
@export var summons_per_wave    : int   = 5
@export var max_minions         : int   = 12
...
func _physics_process(delta: float) -> void:
	...
	if not _is_enraged and current_hp <= max_hp * 0.4:
		_trigger_enrage()
...
func _trigger_enrage() -> void:
	...
	summon_interval   = 1.4
	summons_per_wave  = 6
	attack1_interval  = 1.2
	attack2_interval  = 2.2
	vulnerable_duration = 2.2
	# Flash red to signal enrage
	var tween := create_tween()
	tween.tween_property(self, "modulate", Color(2.0, 0.2, 0.2), 0.1)
...
func _enter_shoot_phase() -> void:
	...
	_shoot_timer   = 0.0
	_action_timer  = 0.0
	_attack1_timer = attack1_interval * 0.2
	_attack2_timer = attack2_interval * 0.2
	_pick_fly_action()
	weakpoint.set_deferred("monitoring",  false)
	weakpoint.set_deferred("monitorable", false)
	eye_sprite.play("fly")
...
func _spawn_minions() -> void:
	...
	for i in to_spawn:
		...
		var offset := Vector2(randf_range(-80.0, 80.0), 0.0)
...
func _handle_attack_timers(delta: float) -> void:
	...
	if _attack1_timer <= 0.0:
		_attack1_timer = attack1_interval
		if _is_enraged:
			# Triple shot in enrage — three bullets fanned slightly
			_shoot_bullet(attack1_root.global_position)
			_shooter1.shoot_at(attack1_root.global_position, player.global_position + Vector2(-30, 0))
			_shooter1.shoot_at(attack1_root.global_position, player.global_position + Vector2( 30, 0))
		else:
			_shoot_bullet(attack1_root.global_position)
...
func _shoot_spread() -> void:
	if _shooter2 == null:
		return
	# 6 projectiles in enrage, 4 normally
	var offsets : Array[Vector2]
	if _is_enraged:
		offsets = [
			Vector2(-attack2_spread * 1.0,  0.0),
			Vector2(-attack2_spread * 0.6,  0.0),
			Vector2(-attack2_spread * 0.2,  0.0),
			Vector2( attack2_spread * 0.2,  0.0),
			Vector2( attack2_spread * 0.6,  0.0),
			Vector2( attack2_spread * 1.0,  0.0),
		]
